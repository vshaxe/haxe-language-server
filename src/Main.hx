...
class Main {
    static function main() {
        ...
        proto.onDocumentSymbols = function(params, cancelToken, resolve, reject) {
            var doc = context.getDocument(params.textDocument.uri);
            var filePath = uriToFsPath(params.textDocument.uri);
            var args = [
            ...
            context.callDisplay(args, stdin, cancelToken, function(data) {
                ...
                for (v in data) {
                    ...
                    if (pos == null) {
                        proto.sendShowMessage({type: Error, message: "Couldn't parse position for " + haxe.Json.stringify(v)});
                        continue;
                    }
